...
(defun huangxiang/directory-parent (directory)
...
;; Screenshot
(defun huangxiang//insert-org-or-md-img-link (prefix imagename)
  (if (equal (file-name-extension (buffer-file-name)) "org")
      (insert (format "[[%s%s]]" prefix imagename))
    (insert (format "![%s](%s%s)" imagename prefix imagename))))

(defun huangxiang/capture-screenshot (basename)
  ...
  (if (file-exists-p (file-name-directory fullpath))
      (progn
        ...
        (huangxiang//insert-org-or-md-img-link "../img/" relativepath))
    (progn
      (call-process "screencapture" nil nil nil "-s" (concat basename ".png"))
      (huangxiang//insert-org-or-md-img-link "./" (concat basename ".png"))))
  (insert "\n"))

(defun huangxiang/org-archive-done-tasks ()
  (interactive)
  (org-map-entries
   (lambda ()
     (org-archive-subtree)
...
(defun huangxiang/org-archive-cancel-tasks ()
  (interactive)
  (org-map-entries
   (lambda ()
     (org-archive-subtree)
...
(defun huangxiang/skip-non-stuck-projects ()
...
(defun huangxiang/org-insert-src-block (src-code-type)
...
(defun huangxiang/org-summary-todo (n-done n-not-done)
  "Switch entry to DONE when all subentries are done, to TODO otherwise."
  (let (org-log-done org-log-states)    ; turn off logging
    (org-todo (if (= n-not-done 0) "DONE" "TODO"))))

(defun huangxiang/filter-by-tags ()
  (let ((head-tags (org-get-tags-at)))
    (member current-tag head-tags)))

(defun huangxiang/org-clock-sum-today-by-tags (timerange &optional tstart tend noinsert)
  (interactive "P")
    ...
    (while (setq file (pop files))
      ...
      (with-current-buffer org-agenda-buffer
        (dolist (current-tag include-tags)
          (org-clock-sum tstart tend 'huangxiang/filter-by-tags)
