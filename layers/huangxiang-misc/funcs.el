;;; funcs.el --- huangxiang Layer packages File for Spacemacs
;;
;; Copyright (c) 2015-2016 huangxiang 
;;
...
(defun huangxiang/highlight-dwim ()
  (interactive)
  (if (use-region-p)
      (progn
...
(defun huangxiang/clearn-highlight ()
    (interactive)
...
(defun huangxiang/insert-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ";")))

(defun huangxiang/delete-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ";")
...
(defun huangxiang/insert-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ",")))

(defun huangxiang/delete-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ",")
...
(defun huangxiang/load-my-layout ()
  (interactive)
  (persp-load-state-from-file (concat persp-save-dir "huangxiang")))

(defun huangxiang/save-my-layout ()
  (interactive)
  (persp-save-state-to-file (concat persp-save-dir "huangxiang")))
...
(defun huangxiang/indent-region(numSpaces)
  (progn
...
(defun huangxiang/tab-region (N)
  (interactive "p")
  (if (use-region-p)
      (huangxiang/indent-region 4)               ; region was selected, call indent-region
    (insert "    ")                   ; else insert four spaces as expected
    ))

(defun huangxiang/untab-region (N)
  (interactive "p")
  (huangxiang/indent-region -4))

(defun huangxiang/hack-tab-key ()
  (interactive)
  (local-set-key (kbd "<tab>") 'huangxiang/tab-region)
  (local-set-key (kbd "<S-tab>") 'huangxiang/untab-region)
  )

;; I'm don't like this settings too much.
;; (add-hook 'prog-mode-hook 'huangxiang/hack-tab-key)
...
(defun huangxiang/helm-hotspots ()
  "helm interface to my hotspots, which includes my locations,
org-files and bookmarks"
  (interactive)
  (helm :buffer "*helm: utities*"
        :sources `(,(huangxiang//hotspots-sources))))

(defun huangxiang//hotspots-sources ()
  ...
  `((name . "Mail and News")
    (candidates . (("Calendar" . (lambda ()  (browse-url "https://www.google.com/calendar/render")))
                   ...
                   ("Run current flie" . (lambda () (huangxiang/run-current-file)))
...
(defun huangxiang/now ()
  "Insert string for the current time formatted like '2:34 PM'."
  (interactive)                 ; permit invocation in minibuffer
  (insert (format-time-string "%D %-I:%M %p")))

(defun huangxiang/today ()
...
(defun huangxiang/browser-refresh--chrome-applescript ()
  (interactive)
  (do-applescript
   (format
    "
...
(defun huangxiang/open-file-with-projectile-or-counsel-git ()
  (interactive)
  (if (huangxiang/git-project-root)
      (counsel-git)
    (if (projectile-project-p)
        (projectile-find-file)
      (counsel-file-jump))))

(defun huangxiang/pomodoro-notification ()
  ...
  (if (spacemacs/system-is-mswindows)
    ...
    (progn (add-hook 'org-pomodoro-finished-hook '(lambda () (huangxiang/growl-notification "Pomodoro Finished" "☕️ Have a break!" t)))
             (add-hook 'org-pomodoro-short-break-finished-hook '(lambda () (huangxiang/growl-notification "Short Break" "🐝 Ready to Go?" t)))
             (add-hook 'org-pomodoro-long-break-finished-hook '(lambda () (huangxiang/growl-notification "Long Break" " 💪 Ready to Go?" t))))))

;; http://blog.lojic.com/2009/08/06/send-growl-notifications-from-carbon-emacs-on-osx/
(defun huangxiang/growl-notification (title message &optional sticky)
...
(defun huangxiang/growl-timer (minutes message)
  ...
  (run-at-time (* (string-to-number minutes) 60)
               ...
               (lambda (minute message)
                 (huangxiang/growl-notification "Emacs Reminder" message t))
               minutes
               message))

(defun huangxiang/goto-match-paren (arg)
...
(defun huangxiang/hidden-dos-eol ()
  "Do not show ^M in files containing mixed UNIX and DOS line endings."
  (interactive)
  (setq buffer-display-table (make-display-table))
  (aset buffer-display-table ?\^M []))

(defun huangxiang/remove-dos-eol ()
  "Replace DOS eolns CR LF with Unix eolns CR"
  (interactive)
  (goto-char (point-min))
  (while (search-forward "\r" nil t) (replace-match "")))

(defun huangxiang/insert-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
  (insert (huangxiang/retrieve-chrome-current-tab-url)))

(defun huangxiang/retrieve-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
...
(defun huangxiang/single-lines-only ()
...
(defun huangxiang/named-term (name)
  (interactive "sName: ")
  (ansi-term "/bin/zsh" name))


(defun huangxiang/ash-term-hooks ()
...
(defalias 'tt 'huangxiang/terminal)
...
(defun huangxiang/word-count-for-chinese ()
  "「較精確地」統計中/日/英文字數。
- 文章中的註解不算在字數內。
...
(defun huangxiang/evil-quick-replace (beg end )
  (interactive "r")
...
(defun huangxiang/git-project-root ()
...
(defun huangxiang/iterm-shell-command (command &optional prefix)
  "cd to `default-directory' then run COMMAND in iTerm.
With PREFIX, cd to project root."
  (interactive (list (read-shell-command
                      "iTerm Shell Command: ")
                     current-prefix-arg))
  (let* ((dir (if prefix (huangxiang/git-project-root)
                default-directory))
...
(defun huangxiang/my-mc-mark-next-like-this ()
  (interactive)
  (if (region-active-p)
...
(defun my-open-file-in-external-app (file)
  ...
  (interactive)
  (let ((default-directory (huangxiang/git-project-root))
        (file-path file))
    (if file-path
        (cond
...
(defun huangxiang/magit-visit-pull-request ()
...
(defun huangxiang/markdown-to-html ()
  (interactive)
...
(defun huangxiang/github-browse-commit ()
...
(defun huangxiang/search-in-fireball ()
  (interactive)
  (helm-do-ag (expand-file-name "~/Github/fireball/")))


(defun huangxiang/show-current-buffer-major-mode ()
  (interactive)
  (describe-variable 'major-mode))

(defun huangxiang/counsel-imenu ()
  (interactive)
  (counsel-imenu)
  (evil-set-jump))
