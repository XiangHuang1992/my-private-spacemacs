...
(add-hook 'term-mode-hook 'huangxiang/ash-term-hooks)
...
(defun huangxiang/untabify-buffer ()
  (interactive)
  (save-excursion
...
(add-hook 'c++-mode-hook
          #'(lambda ()
             (add-hook 'write-contents-hooks
                       'huangxiang/untabify-buffer nil t)))
...
(defmacro huangxiang|toggle-company-backends (backend)
  "Push or delete the backend to company-backends"
  (let ((funsymbol (intern (format "huangxiang/company-toggle-%S" backend))))
    `(defun ,funsymbol ()
       (interactive)
